...
function show_statistics(
    ...
    total_individual_population::Int,
...
"""
function update_dfs!(
    aggregated_idividuals::DataFrame,
    disaggregated_households::DataFrame,
    available_individuals::DataFrame,
...
"""
function select_household_head!(
    aggregated_individuals::DataFrame,
    disaggregated_households::DataFrame,
    hh_size::Int,
    household_id::Int,
    ...
    if hh_size > 1
        available_heads =
            filter(MARITALSTATUS_COLUMN => ==(AVAILABLE_FOR_MARRIAGE), available_heads)
    else
        available_heads =
            filter(MARITALSTATUS_COLUMN => !=(AVAILABLE_FOR_MARRIAGE), available_heads)
    end
    ...
    individual_index = sample(
        collect(1:nrow(available_heads)),
        Weights(available_heads[:, POPULATION_COLUMN]),
    )
    update_dfs!(
...
function select_partner!(
    ...
    available_partners =
        filter(MARITALSTATUS_COLUMN => ==(AVAILABLE_FOR_MARRIAGE), available_partners)
    ...
    #update disaggregated_households with the selected id
    individual_index = sample(
        collect(1:nrow(available_partners)),
        Weights(available_partners[:, POPULATION_COLUMN]),
    )
    update_dfs!(
        aggregated_individuals,
        disaggregated_households,
...
function select_child!(
    ...
    individual_index = sample(
        collect(1:nrow(available_children)),
        Weights(available_children[:, POPULATION_COLUMN]),
    )
    update_dfs!(
        aggregated_individuals,
        disaggregated_households,
...
"""
function allocate_household_members!(
    disaggregated_households::DataFrame,
    aggregated_individuals::DataFrame,
    hh_size::Int,
)
    ...
    available_households =
        filter(Symbol(HOUSEHOLD_SIZE_COLUMN) => ==(hh_size), available_households)
    ...
    response = select_household_head!(
        aggregated_individuals,
        disaggregated_households,
        hh_size,
        household_id,
...
function spread(vals, cnts)
    [v for (v, c) in zip(vals, cnts) for i = 1:c]
end
...
"""
function assign_individuals_to_households(
    aggregated_individuals::DataFrame,
    aggregated_households::DataFrame;
    ...
    if maximum(disaggregated_households[:, HOUSEHOLD_SIZE_COLUMN]) > 2
        for i in collect(3:maximum(disaggregated_households[:, HOUSEHOLD_SIZE_COLUMN]))
            ...
            disaggregated_households[:, column_name] =
                Int.(zeros(nrow(disaggregated_households)))
        end
    end
    ...
    @showprogress for _ = 1:sum(aggregated_households_df[:, POPULATION_COLUMN])

        #select households which are still available
        aggregated_households_df =
            filter(POPULATION_COLUMN => >(0), aggregated_households_df)
        nrow(aggregated_households_df) == 0 ? break : nothing

        #allocate household members
        row_index = sample(
            collect(1:nrow(aggregated_households_df)),
            Weights(aggregated_households_df[:, POPULATION_COLUMN]),
        )
        hh_size = aggregated_households_df[row_index, HOUSEHOLD_SIZE_COLUMN]
        response = allocate_household_members!(
            disaggregated_households,
    ...
    deleteat!(
        hh_colnames,
        findall(x -> (x in [string(ID_COLUMN), string(POPULATION_COLUMN)]), hh_colnames),
    )
